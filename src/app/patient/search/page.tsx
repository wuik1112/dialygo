...
import { 
  ...
  FiCrosshair, FiAlertCircle, FiPlus, FiTrash2, FiFileText
} from 'react-icons/fi';
...
export default function PatientSearchBooking() {
  ...
  // --- NEW: MULTI-DATE CART & REVIEW STATES ---
  ...
  // Reset cart when branch is deselected
  useEffect(() => {
    if (!selectedBranch) {
  ...
  // Check slots based on the current draftDate
  useEffect(() => {
  ...
  const resetFilters = () => {
    ...
    setSearchTerm(''); setUserLocation(null); setFilteredBranches(branches);
  };
  ...
  const addSessionToCart = () => {
    ...
    setDraftShift(''); // Reset shift selection for next addition
  };
  ...
  return (
    <div className='max-w-md mx-auto bg-slate-50 h-[100dvh] relative shadow-2xl font-sans overflow-hidden flex flex-col'>
      ...
      {/* ========================================= */}
      {/* VIEW 1: SEARCH & FILTER (Hidden if viewing branch) */}
      {/* ========================================= */}
      {!selectedBranch && (
        <>
          <div className='bg-white px-5 pt-12 pb-4 shadow-sm z-10 shrink-0'>
            ...
            <div className='flex gap-2'>
              <div className='relative flex-1'>
                ...
                <input type="text" placeholder="Search centre here" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className='w-full pl-12 pr-4 py-3 bg-slate-100 border-none rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-800 text-sm' />
              </div>
              <button onClick={() => setShowFilters(!showFilters)} className={`px-4 rounded-xl flex items-center justify-center transition-colors ${showFilters || filterRegion || filterShift || filterFacilities.length > 0 || filterPrice ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
            ...
            {showFilters && (
              <div className='mt-4 pt-4 border-t border-slate-100 animate-in slide-in-from-top-2 max-h-[60vh] overflow-y-auto custom-scrollbar'>
                ...
                <div className='mb-5'>
                  <button onClick={handleGetLocation} className={`w-full py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-sm border transition-all ${userLocation ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-blue-200 text-blue-600 hover:bg-blue-50'}`}>
                    <FiCrosshair className={isLocating ? 'animate-spin' : ''} />
                    {isLocating ? 'Finding your location...' : userLocation ? 'Location Acquired (Sorting by Nearest)' : 'Sort by Nearest to Me'}
                  </button>
                </div>
          ...
          <div className='flex-1 overflow-y-auto p-5 pb-24 space-y-4 custom-scrollbar'>
            <div className='flex justify-between items-end mb-2'>
              ...
              {userLocation && <span className='text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full'>Sorted by distance</span>}
            </div>
                  ...
                  <div className='h-32 bg-slate-200 relative'>
                    ...
                    <div className='absolute top-3 left-3 flex flex-col gap-1'>
                      {branch.distance !== undefined && branch.distance < 9999 && (
                        <div className='bg-black/60 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 shadow-sm'>
                          <FiMapPin /> {branch.distance.toFixed(1)} km
                        </div>
                      )}
                    </div>
